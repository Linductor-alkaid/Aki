...
- 2026-09-24：`M4-01` 完成（设计先行，纯文档变更，无产品代码；M4 启动，状态 In Progress）：设计 §7.1「传输集成契约（M4 契约）」固化五条——TransferManager 职责切分（DEC-008 模式扩展）、传输 typed 更新→作业映射补充（暂停/恢复/取消经 UpsertTransfer 幂等 + M2-06 既有作业组，不新增作业类型）、.part 承载与 DatabaseWorker 通道容量复核结论（分块 IO 由传输会话长任务直接写 .part、不经 DB 通道，批上限维持 64×2≤256——DEC-009 触发条款履行）、BLAKE3 wire 校验与 SHA-256 存储哈希层次澄清（无冲突，不立统一决策）、DEC-006 映射 7 实现级细化（FileTransferPhase 八态→Aki 状态映射逐条 + pull_file 预留；DEC-006 已回填）。链接核验 62/62。详见 M4 里程碑文档 M4-01 验证记录。
- 2026-09-24：`M4-02` **受阻未完成**（工程规范 4.3/第 7 节，不冒充完成）：TransferSessionManager 原型（单飞排空泵 + 会话循环 + 七状态机）写入工作树后，最小生命周期探针 100% 复现 SIGSEGV。2026-09-24 评审修正（根因已定位并修复，原条目两处与事实不符）：①「SPI source_path 扩展写入工作树」不成立——M4-01 仅完成设计定案（§7.1⑤/§8.1/DEC-006），全部 `start_file_transfer` 调用点仍为三参签名；「单测初稿」`test_transfer_session_manager.cpp` 已删除（仅残留构建产物），工作树交付物仅为未跟踪头文件本体。② SIGSEGV 根因实证：`start_transfer` 丢弃 `submission.future`（SessionRecord.future 从未赋值），`stop_all` 对非法 future 调 `wait_for` 抛 `future_error(no_state)` 且取消未下达（句柄随 clear 丢失→孤儿会话）；次生缺陷：`consume_settled_futures` 不复位在飞代号（单飞永久卡死、命令静默滞留）。修复：future 随句柄保存、`stop_all` 沿 M3-07 次序重写、消费已结算排空 future 时复位代号；MSVC 14.44.35207 同工具链探针连跑 4 次 `consumed=1`+`fully_stopped=1`（修复前同探针 `stop_all THREW: no state`+shutdown 停滞），GCC -Werror 语法检查通过。M4 里程碑保持 In Progress；M4-02 待重建单测（DOD-02 六项 + 七状态机/RULE-08）后全量验收。详见 M4 里程碑文档 M4-02 评审修正条目。
...
- 2026-09-25：`M4-02` 完成（受阻条目闭环；M4 保持 In Progress）：SIGSEGV 根因
  独立复核闭环——三探针（`build/sigsegv-verify/`，gitignored）实证 executor
  对终态任务取消 2500 次全干净（契约 noexcept + 过期句柄幂等 + registry
  map/tombstone 实现 + 上游幂等测试覆盖），「request_task_cancel 不稳定」
  系误诊且逻辑上不成立（原缺陷路径该调用不可达）；本轮单测期新 SIGSEGV
  定位为 submit→登记窗口的孤儿会话同族机制并以 lifecycle 互斥闭合。
  实现：`transfer/manager/transfer_session_manager.hpp` 按 §7.1① 重构——
  出站四接口泵上串行（无幽灵 Queued）、同步拒绝可见（收件箱满/提交即拒）、
  状态事件仅实际变化投递一次、`start_gate` 保证事件顺序、`drain_loop`
  单消费者不变量（MpscChannel 契约）、`stop_all` 全局预算 + 活动态终态
  上报 + 未消费 future 析构兜底；SPI `start_file_transfer` source_path
  签名扩展（§7.1⑤）Fake/真实 Adapter/app TransferManager 全调用点同步；
  单测重建 test_transfer_session_manager 9 用例 86 断言（DOD-02 六项 +
  七状态机/RULE-08 + 重复 ID/收件箱满拒绝）。验证：debug/release 全量
  ctest 32/32 零回归，新测试二进制随机顺序 30 连跑稳定；ASAN/UBSAN/TSAN
  随本 PR CI。观察登记：manager_runtime.hpp drain_loop 同类单消费者窗口
...
- 2026-09-26：`M4-07` 完成（收口审计与退出证据归集，纯审计与文档，无产品
  代码）：设计-实现审计矩阵逐项一致（§6/§6.1/§7/§7.1①~⑥/§8.1/§8.3/
  §10.1/§11.1①~④/§14 与实现 + DEC-004~013 逐一；M4-03~06 四项已知偏差
  锚点核实，未记录偏差数 0）；RULE-07/RULE-10 边界 grep 通过（第一方无线程
  创建、sqlite3 封死 persistence、领域面零 heyaki/executor include——codec
  仅注释命中）；退出-1~5 证据归集完整（DOD-02/状态机用例映射逐项、链接
  复跑 219/0）；本地 debug/release 全量 ctest 复跑 38/38；gh 逐 PR 核实
  #30~#37 CI 五档全绿（#38 为本项 PR）。退出-1 沿 M3-09 先例部分验证 +
