# ZCode Design System（直接采用的 Aki UI 约束）

> 状态：Active（本文件为 Aki UI 的权威约束文档，[Aki UI 设计规范](aki_ui_design.md)给出
> 其在 EUI-NEO 上的绑定映射）
> 来源：[zai-org/ZCode](https://github.com/zai-org/ZCode) `DESIGN.md`，
> commit `872ad960de7ec172591f7e1952f7849229f94521`（2026-09-21 抓取）
> 许可：Apache License 2.0（上游仓库 LICENSE）。以下正文为上游原文，仅格式适配，
> 未改动约束内容；升级时替换正文并更新本头部的 commit。
