// M3-04：设备发现与信任真实化集成测试（DEC-006 映射 2/3；SCOPE-02/03；
// 设计第 4/8.1/8.2/8.3 节；双节点进程内回环）。
//
// 覆盖：
//   - 借用注入（DEC-006）：两 Node 共享一个 borrowed Runtime（挂在
//     ExecutorOwner 的 executor 上）；关闭顺序 Node::shutdown →
//     Runtime::shutdown 断言 executor_shutdown_performed == false +
//     owner fully_stopped；未运行 executor 的 create_borrowed 以
...
//   - 已知设备持久化：Trusted 行经写路径落库，重启恢复后仍 Trusted 且
//     trusted 端点不再合成 discovered（验收 ③ + §8.1 已知设备不重放）。
//   - DOD-02 六项沿新并发路径（periodic 轮询 + pairing observer）：本文件
//     覆盖正常完成（回环）、shutdown（借用关闭断言 + pipeline stop）、
//     执行中取消与提交拒绝（executor 周期能力专项 TEST_CASE）、超时（有界
//     等待预算耗尽可见）。
//
// 环境限制：真实 LAN 多播需要可用网络接口——接口缺失时本文件输出
// "[skip] no LAN interface" 并以通过结束（回环验证留待有接口环境补跑，
// 见 M3 里程碑验证记录补跑条件），不冒充已验证。
