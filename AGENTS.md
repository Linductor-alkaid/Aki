...
本文件适用于 Aki 仓库中的全部自研代码、测试、文档与构建配置。`third_party/`
...
`Aki` 是使用现代 C++ 构建的、运行在 Heyaki 设备通信网络之上的跨设备即时通信客户端
（Heyaki 的 reference application）。它把电脑、手机、服务器、机器人等运行 Heyaki 的
终端统一表示为 `Device`，为多设备之间的通信提供统一入口；聊天窗口同时是设备交互入口，
第一阶段处理文本、图片、视频和文件。核心闭环为：

`建立设备身份 -> 发现并信任设备 -> 建立 Conversation -> 收发文本/图片/文件 -> 历史持久化与断线恢复`

实现必须保持 Core 与具体平台/传输/供应商解耦：Heyaki、EUI-NEO、操作系统等宿主能力只能
通过 Adapter/Provider 接入，不得渗入 Core。优先围绕以下边界形成稳定接口：

- `Heyaki Adapter`：把 Heyaki 的身份、发现、信令、P2P/Relay 建链与传输能力转换为设备
  发现、消息发送、文件传输等应用语义，使 Manager 与 UI 不直接依赖 Heyaki 接口，
  Heyaki 的接口变化收敛在这一层。
- `Application State`：Heyaki 异步事件先进入对应 Manager 并更新 Application State，
  UI 只消费状态变化；网络线程与渲染线程之间以该状态边界隔离。
- Typed `Message` / `Transfer` 模型与 Capability/Permission 边界：为设备状态、命令、
  终端、Agent 等后续能力提供可发现、可授权、可验证的扩展点。

## Executor 是强制并发基础设施

Aki 必须依赖仓库中的 pinned `third_party/executor` 管理所有并发任务和运行生命周期。
...
5. 长期阻塞 I/O 使用 Executor 的 blocking worker 生命周期能力；允许抖动的后台周期任务使用
   ...
   使用 realtime/low-latency 能力。不得用普通周期任务冒充实时控制。
...
10. 队列容量（含 `max_in_flight_tasks` 总量准入）、背压/drop 策略、超时、取消和关闭中的提交
    必须转化为明确结果与事件，不得静默重试、无限排队或吞掉失败。
11. 第三方库回调只做有界校验和投递，业务 handler 不得直接在第三方线程执行；经第三方事件
    循环的延续派发须遵守集成指南的豁免纪律并保持可追溯。
...
满足 Aki 的合理需求时，必须按工程规范第 9.4 节执行反馈流程：
...
以下情况不是 Executor 能力缺口：Heyaki 协议与 EUI-NEO 适配、Windows/Linux/Android
平台映射、业务状态机策略、错误使用已有 API、平台本身不提供所需权限。它们应在 Aki
对应层解决。

## Runtime 与状态模型

- 核心对象必须实现为可推进、可观测、可中断的显式状态机，而不是不可中断的单体 `run()`。
  标准状态集以[设计文档](docs/design/aki_design.md)为准：信任状态
  `Unknown / Pending / Trusted / Rejected / Revoked`，传输状态
  `Queued / Negotiating / Transferring / Paused / Completed / Failed / Cancelled`，
  以及 `PresenceState`、`ConversationState` 和消息 `DeliveryState`。新增状态或转换必须
  先更新设计文档，不允许出现代码私有状态。
- 每个推进步骤应是有界工作单元；耗时网络请求、I/O 和控制循环交给 Executor 管理，并通过
  结构化结果推动状态转换。
- Task 与 Session 必须有稳定 ID、取消上下文和生命周期所有者。终态必须幂等；迟到的响应或
  动作结果不得让已取消/已完成任务重新进入活动状态。

## 工程约束

- 使用 C++20 和 CMake（≥ 3.25）+ `CMakePresets.json`；公开 API 避免暴露平台与第三方类型，
  平台相关编译单元保持可选。依赖方向指向抽象，Adapter 依赖 Core 接口，不得反向依赖。
- 所有跨线程共享状态必须有明确所有权或使用 Executor 提供的通信原语；不得依赖隐式全局
  可变状态。所有队列、缓存和并发 operation 都有容量或预算上限。
- 为状态转换、取消竞态、关闭顺序、队列拒绝、协议解析和边界校验编写测试。新增并发路径时
...
- Commit Message 使用 `<type>(<scope>): <subject>`；type 限于
  ...
  仓库约定的模块词表。一个 Commit 对应一个独立逻辑修改；含义不明确的提交说明不可接受。
...
一项 Aki 变更只有在以下条件满足时才算完成：职责位于正确层；全部任务受 Executor
